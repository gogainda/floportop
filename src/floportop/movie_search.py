...
MODEL_DIR = CACHE_DIR / "search_model"
...
def load_movie_data(force_rebuild=False):
    ...
    if not force_rebuild and _movies_df is not None:
        return _movies_df
    ...
    if not force_rebuild and MOVIES_PKL.exists():
        ...
        print(f"✅ Loaded cached data: {len(_movies_df):,} movies")
        return _movies_df
...
def load_model():
    ...
    if MODEL_DIR.exists():
        _search_model = SentenceTransformer(str(MODEL_DIR))
        return _search_model

    print("🧠 Downloading embedding model...")
    _search_model = SentenceTransformer(MODEL_NAME)
    _search_model.save(str(MODEL_DIR))
    return _search_model
...
def build_index(movies_df):
    ...
    _search_index = faiss.IndexFlatIP(embeddings.shape[1])
    _search_index.add(embeddings)

    faiss.write_index(_search_index, str(INDEX_FAISS))
    print(f"✅ Index built: {_search_index.ntotal:,} movies")
...
def load_index():
    ...
    _search_index = faiss.read_index(str(INDEX_FAISS))
    return _search_index
